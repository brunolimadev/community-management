...
public class CreditCardValidator {
	...
	public static boolean validateExpirationDate(String dataValidade) {
		...
		if (partes.length != 2 || partes[0].length()  !=2 || partes[1].length() != 2) {
			throw new ValidacaoException("Pagamento não aprocado: Formato de data inválido");
		}
		...
		if(mes > 12 || mes < 1) {
			throw new ValidacaoException("Pagamento não aprocado: Mes/Ano informado inexistente!");
		}
		...
		if (ano < anoAtual || (ano == anoAtual && mes < java.time.MonthDay.now().getMonthValue())) {
			throw new ValidacaoException("Pagamento não aprocado: Data de validade expirada!");
		}
		return true;
	}
	...
	public static boolean validatePix(String numero) {
		...
		if (Objects.isNull(numero) || numero.length() == 0 ) {
			throw new ValidacaoException("Pagamento não Aprovado: Código validação pix não enviado!");
		}
		return true;
	}
